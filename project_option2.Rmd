...
```
```{r}
x <- model.matrix(red_model)
# Find the median of each column and store in x0
(x0 <- apply(x,2,median))
# Find hat y0 of x0
(y0 <- sum(x0*coef(red_model)))
# # which is the same as
# predict(red_model,new=data.frame(t(x0)))
# 
# # Let's consider the mean house on the market and predict the confidence intervals
# 
# # for the mean value:
# predict(red_model,new=data.frame(t(x0)),interval="confidence")
# 
# # and for new observations:
# predict(red_model,new=data.frame(t(x0)),interval="prediction")
# 
# # confidence intervals for individual beta_j's
# confint(red_model)
```
